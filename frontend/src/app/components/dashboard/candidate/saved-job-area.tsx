    ...
    const fetchSavedJobs = async () => {
      try {
        ...
        const response = await axios.get(
          `http://a80ab3f4d07d1472e9703892bfd831e4-2032056606.us-east-1.elb.amazonaws.com:5000/api/auth/getAllSavedJobs`,
          {
            headers: {
    ...
    const fetchJobDetails = async () => {
      const promises = savedJobs.map(async (savedJob) => {
        try {
          const token = localStorage.getItem("token");
          const response = await axios.get(
            `http://a80ab3f4d07d1472e9703892bfd831e4-2032056606.us-east-1.elb.amazonaws.com:5000/api/auth/getJobDetailsUsingSavedJobs/${savedJob.job_id}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
