...
const CompanyDetailsArea = () => {
  ...
  useEffect(() => {
    const fetchCompanyDetails = async () => {
      try {
        if(id){
          const response = await axios.get(`http://100.25.158.124:5000/api/auth/getCompanyDetailsUsingId/${id}`);
    ...
    const fetchProfilePicture = async () => {
      try {
        const response = await axios.get(`http://100.25.158.124:5000/api/auth/getCompanyProfilePictureUsingId/${id}`);
